...
import React from "react";
import { FaHeart, FaArrowRight, FaStar, FaExchangeAlt, FaClock } from "react-icons/fa";
import { HiLocationMarker } from "react-icons/hi";

const TopFlight = (props) => {
    return (
        <div className="flex flex-col sm:flex-row w-[650px] sm:w-auto rounded-[20px] overflow-hidden border-[1px] border-gray-300 h-auto sm:h-[350px]">
            {/* Left side: Image and Rating */}
            <div className="relative w-full sm:w-[45%] h-[250px] sm:h-full rounded-[20px] overflow-hidden">
                <img
                    src={props.data.imageUrl}
                    alt="destination"
                    className="w-full h-full object-cover"
            ...
            {/* Right side: Details */}
            <div className="w-full sm:w-[55%] p-4 flex flex-col justify-between h-auto sm:h-full">
                <div className="flex-grow">
                    <div className="flex flex-row sm:flex-row items-center sm:ml-8 gap-3">
                        {/* Departure Date with Time Icon */}
                        <div className="flex items-center gap-1">
                        ...
                        {/* Vertical Line */}
                        <div className=" border-t-2 border-gray-500 my-2 w-[2vw]" />
                    ...
                    <div className="flex flex-row sm:flex-row gap-4 sm:ml-8 mt-2">
                        <h1 className="text-[25px] font-bold">{props.data.startLocation}</h1>
                    ...
                    <div className="flex items-center text-gray-500 gap-1 mb-2 sm:ml-8">
                        <HiLocationMarker />
                        <h3>{props.data.startCountry}</h3>
                    </div>

                    {/* Flight Information */}
                    <p className="text-xl text-gray-500 sm:ml-8">{props.data.classType}</p>
                    <div className="flex justify-between items-center text-xl sm:ml-8 mt-2">
                        <p className="font-bold text-[18px]">${props.data.price}</p>
                    </div>
                    <p className="text-sm text-gray-500 sm:ml-8 mt-2">{props.data.seatsLeft} Seats left</p>
                </div>
