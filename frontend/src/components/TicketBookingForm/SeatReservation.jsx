...
const SeatReservation = ({
  ...
  const renderSeats = (row) => {
    return seats[row].map((seat) => (
      <div
        key={seat}
        className={`seatContainer ${
          selectedSeats[row]?.includes(seat) ? "selectedSeats" :
          bookedSeats.includes(row + seat) ? "bookedSeats" : "seatsHover"
        }`}
        onClick={() => handleSeatClick(row, seat)}
      >
        <p className="text-[15px]">
  ...
  const numPassengersText = `${Object.values(selectedSeats).reduce((total, seats) => total + seats.length, 0)} Passenger(s)`;
      ...
      <div className="flex flex-col-reverse md:flex-row mt-5">
        ...
        {/* <div className="w-full  hidden md:block">
          <img
            ...
            src={AirplaneHead}
            alt=""
            className=" h-full rotate-[270deg] md:rotate-0"
          />
        </div> */}
{/*  */}
<div className="w-[150vw] hidden md:block mt-[3vw] ">
  <img
    ...
    style={{ height: `${containerSize.height + 160}px` }} // Adjusted height to fit additional rows
  />
</div>
